...
export const TENANT_COLLECTION_FEATURES = {
  ...
  // Legacy commerce collections. NOTE (Wave F2): the runtime orchestration that wrote these is retired;
  // the registrations stay for multi-tenant-plugin relationship sanitization until the legacy slugs are
  // fully unregistered. Gated on `commerce` so they disappear from admin nav when the feature is off.
  ...
  // Plugin-first commerce catalog collections (Wave B4) — the live product/variant/cart/order surface
  // generated by @payloadcms/plugin-ecommerce under collision-free `store-*` slugs. All gated on
  // `commerce` and tenant-scoped. Added 2026-07-21: previously omitted, which meant the `commerce`
  // feature flag did not actually hide the catalog surface for non-commerce tenants — only the legacy
  // slugs above were gated. The shopper-facing surface was already gated at the edge by
  // `resolveStoreTenant` (cms/src/commerce/store/shared.ts), so this is the admin/API capability gap.
