  ...
  for (let i = 0; i < attempts; i++) {
    try {
      return await drizzleOf(payload).transaction(async (tx) => {
        const upd = await tx.run(sql`UPDATE \`commerce_settings\` SET \`order_number_seq\` = \`order_number_seq\` + 1 WHERE \`tenant_id\` = ${tenantId}`)
        ...
        return `${prefix}${row!.order_number_seq}`
      })
    } catch (err) {
      lastErr = err
