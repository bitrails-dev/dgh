...
const FIELDS = ['name', 'name_ar', 'description', 'description_ar', 'icon', 'center_of_excellence', 'featured', 'image'] as const;
...
export async function getDepartment(_req: Request, env: Env, params: Record<string, string>): Promise<Response> {
...
export async function updateDepartment(request: Request, env: Env, params: Record<string, string>): Promise<Response> {
  const body = await request.json() as Record<string, unknown>;
  const sets: string[] = [];
  const vals: unknown[] = [];
...
export async function deleteDepartment(_req: Request, env: Env, params: Record<string, string>): Promise<Response> {
