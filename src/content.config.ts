...
const articles = defineCollection({
  loader: async () => {
    ...
    return docs.map((doc) => {
      ...
      return { id: doc.slug, title, titleAr, date: new Date(doc.date), author: str(doc.author), category, categoryName, categoryNameAr, categoryColor, thumbnail: imgUrl(doc.thumbnail), featured: doc.featured ?? false, content: normalizeBlocks(doc.content) };
    });
  },
  schema: z.object({
    title: z.string(),
